...
export default function Page3({ accumulatedData, onUpdate, onBack, onNext }: Props) {
  ...
  const styles: { [key: string]: React.CSSProperties } = {
    contenedorMaestro: { backgroundColor: '#ffffff', width: '210mm', minHeight: '279mm', border: '3.5px solid #2c5697', borderRadius: '25px', padding: '12px', boxSizing: 'border-box', position: 'relative', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', },
    tablaGeneral: { width: '100%', borderCollapse: 'collapse', marginBottom: '8px', tableLayout: 'fixed', },
    thTd: { border: '1.5px solid #2c5697', padding: '2.5px', textAlign: 'center', verticalAlign: 'middle', fontSize: '7.8px', lineHeight: '1.1', },
    tituloSeccionAzul: { backgroundColor: '#2c5697', color: '#ffffff', fontWeight: 'bold', fontSize: '12px', padding: '4px 20px', borderRadius: '15px', display: 'inline-block', marginBottom: '8px', marginTop: '2px', width: 'fit-content' },
    encabezadoTablaAzul: { backgroundColor: '#2c5697', color: '#ffffff', fontWeight: 'bold', fontSize: '8.5px', },
    celdaEnfasisGris: { backgroundColor: '#f2f5f9', fontWeight: 'bold', },
    layoutFilaSuperior: { display: 'grid', gridTemplateColumns: '2.1fr 1fr', gap: '12px', width: '100%', },
    layoutFilaMediaRecortada: { display: 'grid', gridTemplateColumns: '3fr 1.1fr', gap: '10px', marginTop: '2px', marginBottom: '0', height: '185px', overflow: 'visible', },
    cajaRecordatorioPrincipalRecortada: { border: '2px solid #2c5697', borderRadius: '18px', height: '100%', overflow: 'hidden', },
    headerDatosRecordatorio: { borderBottom: '1.5px solid #2c5697', padding: '6px', display: 'flex', fontSize: '9px', gap: '15px', },
    contenedorAlimentosOlvidadosVertical: { border: '2px solid #2c5697', borderRadius: '15px', padding: '8px', backgroundColor: '#ffffff', display: 'flex', flexDirection: 'column', height: 'fit-content', boxSizing: 'border-box', },
    layoutFilaInferiorAcomodada: { display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: '12px', marginTop: '65px', },
    seccionEvaluacionCualitativa: { border: '2px solid #2c5697', borderRadius: '15px', padding: '8px', marginTop: '2px', },
    lineaChecklist: { display: 'flex', alignItems: 'center', marginBottom: '3px', fontSize: '9px', },
    piePaginaTexto: { marginTop: 'auto', fontSize: '7.5px', display: 'flex', justifyContent: 'space-between', paddingBottom: '2px', },
    alignLeftPadding: { textAlign: 'left', paddingLeft: '6px', }
  };
  ...
  return (
    <>
      <style>
        {`
          :root { --pdf-grey: #525659; --primary-blue: #1d4d96; --btn-cobalt: #1d4d96; --brand-blue: #2c5697; }
          html, body { margin: 0; padding: 0; background-color: var(--pdf-grey) !important; min-height: 100%; }
          @page { size: letter; margin: 0; }
          @media print { html, body { background-color: #ffffff !important; } * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; } .form-container { background-color: #ffffff !important; padding: 0 !important; } .contenedor-maestro-perimetral { box-shadow: none !important; border: 3.5px solid var(--brand-blue) !important; margin: 0 !important; background-color: #ffffff !important; -webkit-print-color-adjust: exact !important; } div[style*="backgroundColor: rgb(44, 86, 151)"], div[style*="background-color: #2c5697"], tr[style*="backgroundColor: rgb(44, 86, 151)"], .encabezado-azul-print { background-color: #2c5697 !important; color: white !important; } tr[style*="backgroundColor: rgb(242, 245, 249)"], td[style*="backgroundColor: rgb(242, 245, 249)"] { background-color: #f2f5f9 !important; } input { border: none !important; outline: none !important; } .nav-button { display: none !important; } }
          .form-container { font-family: 'Segoe UI', Arial, sans-serif; margin: 0; padding: 40px 20px; background-color: var(--pdf-grey); display: flex; flex-direction: column; align-items: center; min-height: 100vh; position: relative; box-sizing: border-box; }
          .nav-button { position: fixed; bottom: 30px; padding: 14px 32px; border-radius: 50px; font-weight: bold; cursor: pointer; border: none; box-shadow: 0 4px 15px rgba(0,0,0,0.6); transition: all 0.2s; color: white; background-color: var(--btn-cobalt); text-transform: uppercase; font-size: 13px; letter-spacing: 0.5px; z-index: 1000; }
          .btn-back { left: 40px; } .btn-next { right: 40px; }
          .nav-button:hover { transform: translateY(-3px); background-color: #153a71; box-shadow: 0 8px 20px rgba(0,0,0,0.7); }
          input { width: 100%; border: none; background: transparent; font-family: inherit; font-size: inherit; color: #2c5697; outline: none; padding: 0; margin: 0; }
          input[type="number"] { text-align: center; -moz-appearance: textfield; } input[type="number"]::-webkit-inner-spin-button, input[type="number"]::-webkit-outer-spin-button { -webkit-appearance: none; margin: 0; }
          .linea-escritura { border-bottom: 1px solid #2c5697; text-align: left; padding: 2px 5px; height: 22px; box-sizing: border-box; }
          .espacio-blanco-separador { height: 15px; }
          input[type="checkbox"] { cursor: pointer; width: 13px; height: 13px; accent-color: #2c5697; }
          .celda-hallazgo-compacta { padding: 1px 2px !important; height: 14px !important; font-size: 7.2px !important; }
          .contenedor-redondeado-azul { border: 2px solid #2c5697; border-radius: 15px; overflow: hidden; margin-bottom: 8px; }
          .input-unidad { display: flex; alignItems: center; justify-content: center; gap: 2px; }
        `}
      </style>
      
      <div className="form-container">
        <button onClick={onBack} className="nav-button btn-back">← Página 2</button>
        <button onClick={onNext} className="nav-button btn-next">Página 4 →</button>
