/**
 * ============================================================================
 * ARCHIVO: SeguimientoNutricionalCaptura.tsx
 * PROPÓSITO: Interfaz de captura del Seguimiento Nutricional — EXCLUSIVO de
 * nutrición. Fisioterapia usa su propio EvolucionSeguimientoCaptura.tsx, sin
 * el bloqueo por columna que se implementa aquí (ambos dejaron de
 * representar el mismo documento).
 *
 * Cada documento acumula hasta 6 consultas subsecuentes (columnas 1-6); la
 * 7ª subsecuente cierra el ciclo y la 8ª abre un documento nuevo desde la
 * columna 1 (ver seguimientoNutricionalController.js). La columna que le
 * corresponde a la consulta en curso llega ya calculada desde el backend
 * (columnaActual) — un practicante solo puede editarla; las columnas
 * anteriores se muestran en solo lectura. Admin/master no tienen bloqueo.
 * ============================================================================
 */
...
].map((q, idx) => ({ label: q, name: (col: number) => `psi_q${idx}_col${col}`, type: 'textarea' as const, maxLength: 99 }));
...
].map((s, idx) => ({ label: s, name: (col: number) => `sig_${idx}_col${col}`, maxLength: 26 }));
...
const rowsIntervencion: MatrixRowConfig[] = [
  ...
  ...["Energía (kcal y kcal/kg)", "Hidrato de carbono (%/g)", "Proteína (%/g y g/kg/d)", "Lípidos (%/g)"]
    .map((m, idx) => ({ label: m, name: (col: number) => `interv_macro_${idx}_col${col}`, maxLength: 29 })),
  ...["Frutas", "Verduras", "Cereales", "Leguminosas", "POAs _ _ _", "POAs _ _ _", "Lácteos", "Aceites s/p", "Aceites c/p", "Azúcares"]
    .map((eq, idx) => ({ label: eq, name: (col: number) => `interv_eq_${idx}_col${col}`, maxLength: 29 })),
];
...
const rowsFirmas: MatrixRowConfig[] = [
  ...["PLN.", "Matrícula", "Firma", "LN.", "Céd. Prof."].map((label, idx) => ({ label, name: (col: number) => `firma_${idx}_col${col}`, maxLength: 29 })),
  { label: "Firma", name: (col: number) => `firma_final_col${col}`, maxLength: 29 },
];

const SeguimientoNutricionalCaptura = forwardRef<FormClinicoHandle, Partial<FormClinicoCallbacks>>((props, ref) => {
  const navigate = useNavigate();
  const {
    formData, setFormData, isSaving, yaGuardado, setYaGuardado, doSave, canSave, columnaParaBloqueo,
  } = useSeguimientoNutricionalData(props);
  ...
  const { confirmarGuardado } = useFormClinicoController(
    {
      ...
      canSave,
      triggerSave: doSave,
      // Fusiona en vez de reemplazar: las columnas anteriores (solo lectura)
      // ya llegaron por useSeguimientoNutricionalData al montar; el borrador
      // solo trae lo tecleado hasta ahora en la columna actual.
      restoreDraft: (draft) => setFormData(prev => ({
        ...prev,
  ...
  const matrixProps = {
    ...
    columnaActual: columnaParaBloqueo,
  };
  const simpleProps = { formData, onChange: handleInputChange, onDateChange: handleDateInput, filaActual: columnaParaBloqueo };
  const sectionProps = { formData, onChange: handleInputChange, onDateChange: handleDateInput, columnaActual: columnaParaBloqueo };
      ...
      <Tabs defaultValue="p1">
          ...
          <FormSectionCard title="Interpretación bioquímica">
            <SimpleFieldTable
              ...
              columns={[
                ...
                { label: 'Interpretación', name: row => `int_bioq_desc_${row}`, type: 'textarea', maxLength: 352 },
              ]}
              {...simpleProps}
            />
          </FormSectionCard>
        ...
        <TabsContent value="p5" className="space-y-4 mt-4">
          ...
          <FormSectionCard title="Diagnósticos Nutricios">
            <DiagnosticosNutriciosSection {...sectionProps} />
          </FormSectionCard>
        </TabsContent>
        ...
        <TabsContent value="p6" className="space-y-4 mt-4">
          ...
          <FormSectionCard title="Educación Nutricional y Consejería">
            <EducacionConsejeriaSection {...sectionProps} />
          </FormSectionCard>
...
export default SeguimientoNutricionalCaptura;
