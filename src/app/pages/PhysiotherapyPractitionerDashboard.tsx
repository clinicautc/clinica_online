...
export default function PhysiotherapyPractitionerDashboard() {
  ...
  }, [user, estadoFilter, selectedDate, selectedMonth, viewMode]);
              ...
              <CardHeader>
                ...
                <div className="flex flex-wrap gap-1.5 pt-3">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold transition-colors ${citaSubTab === 'en_atencion' ? 'bg-blue-900 text-white' : 'bg-blue-50 text-blue-700 hover:bg-blue-100'}`}
                  >
                    <Calendar className="w-3.5 h-3.5" />
                    Agenda
                    <span className={`text-xs px-1.5 py-0.5 rounded-full font-black ${citaSubTab === 'en_atencion' ? 'bg-white/30 text-white' : 'bg-blue-100 text-blue-800'}`}>
                  ...
                  return displayedAppointments.map((apt) => (
                    <div key={apt.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 border rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow gap-4 border-l-4 border-l-blue-900">
                      ...
                      {apt.estado === 'en_atencion' ? (
                        ...
                        <Button
                          className="h-10.75 px-5 text-base w-full sm:w-auto shadow-sm font-bold text-white bg-blue-900 hover:bg-blue-800"
        ...
        infoBox={
          <>
            ...
            <p className="text-[11px] text-slate-500 mt-1">
              {citaFueraDeHora?.fecha ? format(new Date(citaFueraDeHora.fecha.split('T')[0] + 'T00:00:00'), 'dd/MM/yyyy') : ''} &bull; {citaFueraDeHora?.hora?.substring(0, 5)} &bull; Fisioterapia
            </p>
          </>
        }
        hideConfirm
      ...
      <ConfirmDialog
        ...
        infoBox={
          <>
            ...
            <p className="text-[11px] text-slate-500 mt-1">
              {citaParaIniciar?.fecha ? format(new Date(citaParaIniciar.fecha.split('T')[0] + 'T00:00:00'), 'dd/MM/yyyy') : ''} &bull; {citaParaIniciar?.hora?.substring(0, 5)} &bull; Fisioterapia
            </p>
          </>
        }
        ...
        confirmClass={
          citaParaIniciar?.estado === 'en_atencion'
            ...
            : 'flex-1 bg-blue-600 hover:bg-blue-700 font-bold text-white shadow-md'
        }
