...
export default function ForgotPassword() {
  ...
  return (
      <div className="min-h-screen flex flex-col lg:flex-row bg-gradient-to-br from-blue-50 to-orange-50 font-sans">
        ...
        {/* SECCIÓN IZQUIERDA: INDICADORES DE PASOS */}
          ...
          <div className="max-w-sm w-full">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col items-center">
                <div className={`w-12 h-12 flex items-center justify-center rounded-2xl shadow-sm transition-all
                ${currentStep !== 'email' ? 'bg-blue-900 text-white' : 'bg-white text-blue-900 border-2 border-blue-900 font-bold'}`}>
                  {currentStep !== 'email' ? <Check className="w-5 h-5" /> : '1'}
                </div>
                <span className="text-[10px] mt-2 font-black uppercase text-blue-900/40 tracking-widest">Email</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                <div className={`w-12 h-12 flex items-center justify-center rounded-2xl shadow-sm transition-all
                ${(currentStep === 'password' || currentStep === 'success') ? 'bg-blue-900 text-white' : currentStep === 'code' ? 'bg-orange-500 text-white scale-110' : 'bg-white text-slate-300 border-2 border-slate-100'}`}>
                  {(currentStep === 'password' || currentStep === 'success') ? <Check className="w-5 h-5" /> : '2'}
                </div>
                <span className="text-[10px] mt-2 font-black uppercase text-blue-900/40 tracking-widest">Código</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                <div className={`w-12 h-12 flex items-center justify-center rounded-2xl shadow-sm transition-all
                ${currentStep === 'success' ? 'bg-green-500 text-white' : currentStep === 'password' ? 'bg-orange-500 text-white scale-110' : 'bg-white text-slate-300 border-2 border-slate-100'}`}>
                  {currentStep === 'success' ? <Check className="w-5 h-5" /> : '3'}
                </div>
                <span className="text-[10px] mt-2 font-black uppercase text-blue-900/40 tracking-widest">Nueva</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-md bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border border-white p-8 sm:p-10 space-y-8">
            ...
            {currentStep === 'email' && (
                <form onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    if (res.ok) { setCurrentStep('code'); toast.success("Código enviado"); }
                    else { const d = await res.json(); setEmailError(d.error); }
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input value={email} onChange={(e) => { setEmail(email.trim()); setEmailError(''); }} placeholder="usuario@utc.edu.mx" className="pl-12 h-12 rounded-xl border-slate-100 bg-white shadow-sm" />
                    </div>
                  ...
                  <Button type="submit" disabled={loading} className="w-full h-12 bg-blue-900 hover:bg-black text-white font-black uppercase tracking-widest rounded-xl shadow-lg transition-all active:scale-95">
                    {loading ? <Loader2 className="animate-spin" /> : "Enviar Código de Verificación"}
                  </Button>
                </form>
            )}
            ...
            {currentStep === 'code' && (
                <form onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    else { const d = await res.json(); setCodeError(d.error); }
                  ...
                  <Button type="submit" disabled={loading || verificationCode.length !== 6} className="w-full h-12 bg-orange-600 hover:bg-orange-700 text-white font-black uppercase rounded-xl">
            ...
            {currentStep === 'password' && (
                <form onSubmit={async (e) => {
                  ...
                  <Button type="submit" disabled={loading} className="w-full h-12 bg-green-600 hover:bg-green-700 text-white font-black uppercase rounded-xl">
                    {loading ? <Loader2 className="animate-spin" /> : "Establecer Nueva Contraseña"}
                  </Button>
                </form>
            )}
            ...
            {currentStep === 'success' && (
                <div className="text-center space-y-6 py-4 animate-in zoom-in-95 duration-300">
                  ...
                  <p className="text-slate-500 text-sm font-medium italic">Tu contraseña ha sido actualizada. Ya puedes volver a ingresar al sistema clínico.</p>
