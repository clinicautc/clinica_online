...
import { FileText, Calendar, User, Activity, Utensils, ChevronDown, ChevronUp, Search } from 'lucide-react';
...
export default function MedicalHistoryViewer({ filterType }: MedicalHistoryViewerProps = {}) {
  ...
  useEffect(() => {
    ...
    let loadedHistories = stored ? JSON.parse(stored) : mockMedicalHistories;

    // FILTRO: Si se especifica un tipo, mostrar solo ese tipo
    if (filterType) {
      loadedHistories = loadedHistories.filter((h: MedicalHistory) => h.type === filterType);
    }

    // NOTA: Ordenamos alfabéticamente por nombre de paciente
    loadedHistories.sort((a: MedicalHistory, b: MedicalHistory) => {
      return a.patientName.localeCompare(b.patientName, 'es');
    });

    setAllHistories(loadedHistories);
  ...
  useEffect(() => {
    ...
    const filtered = allHistories.filter((h) => {
      ...
      return (
        ...
        h.createdBy.toLowerCase().includes(searchLower) ||
        h.date.includes(searchTerm)
      );
    });

    setHistories(filtered);
  }, [searchTerm, allHistories]);
  ...
  return (
    <Card className="border-blue-900/10 shadow-md">
      <CardHeader>
        ...
        <CardDescription>
          Expedientes clínicos de Nutrición y Fisioterapia (ordenados alfabéticamente).
        </CardDescription>
        ...
        <div className="relative mt-4">
          ...
          <Input
            type="text"
            placeholder="Buscar por nombre de paciente, creador o fecha..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="pl-10 border-blue-900/20"
          />
        </div>
      </CardHeader>
