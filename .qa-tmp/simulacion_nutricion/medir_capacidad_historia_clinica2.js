...
(async () => {
  ...
  const resultado = await page.evaluate(() => {
    ...
    function etiquetaDe(el) {
      // 1) fila de tabla: usar el texto de la primera celda de la fila
      const tr = el.closest('tr');
      if (tr) {
        const primeraCelda = tr.querySelector('td, th');
        if (primeraCelda && primeraCelda !== el.closest('td, th') && primeraCelda.textContent.trim()) {
          return primeraCelda.textContent.trim().slice(0, 60);
        }
      }
      // 2) hermano anterior con texto (span/label/div)
      ...
      // 3) primer texto dentro del padre antes del propio input
      const parent = el.parentElement;
      if (parent) {
        const clone = parent.cloneNode(true);
        // quitar inputs/textareas clonados para quedarnos solo con texto plano
        clone.querySelectorAll('input, textarea, select').forEach(n => n.remove());
        const t = clone.textContent.trim();
        if (t) return t.slice(0, 60);
      }
      return '(sin etiqueta detectable)';
    }
  ...
  resultado.forEach(f => {
    console.log(
      `${f.tipo.padEnd(9)} max=${String(f.maxCaracteres).padStart(4)} chars | ${f.anchoPx}x${f.altoPx}px @ ${f.fontSize} | x${f.camposDeEsteGrupo} campos | "${f.etiqueta}"`
    );
  });
