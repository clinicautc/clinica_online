...
(async () => {
  ...
  const TABLAS = [
    { nombre: 'sint', prefijoCampo: 'sint_', sufijoCampo: '_col1', pageClass: 'page' },
    { nombre: 'ejer', prefijoCampo: 'ejer_', sufijoCampo: '_col1', pageClass: 'page' },
    { nombre: 'diet', prefijoCampo: 'diet_', sufijoCampo: '_col1', pageClass: 'page' },
  ...
  const resultado = await page.evaluate((tablas) => {
    ...
    for (const cfg of tablas) {
      ...
      let lo = Math.round(alturaOriginal), hi = 90, mejor = Math.round(alturaOriginal);
      while (lo <= hi) {
